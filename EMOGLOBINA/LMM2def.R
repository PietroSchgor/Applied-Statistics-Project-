...
#bwplot(Hb ~ `dose alla fraz mim` , data = dataset)
...
dim(dataset)
# 
# dataset <- dataset[-which(dataset$`dose alla fraz mim` < 1), ]
# dim(dataset)

#dataset <- dataset[-which(dataset$`dose alla fraz mim` > 36), ]
#dim(dataset)
...
# rimozione dei valori di hb == 0
#dataset <- dataset[-which(dataset$Hb == 0), ]
#dim(dataset)
...
sqrt(mse7)
AIC_model7<-AIC(mod7)
#con varIdent diminuiscono AIC e mse 
...
sqrt(mse9) 
AIC_model9<- AIC(mod9)
#includendo correlazione l'mse diminuisce abbastanza mentre l'aic aumenta di poco

anova(mod9, mod8)
#pvalue basso -> il 9 e il 8 sono praticamente equivalenti -> scelgo il 8
# che ha mse e numero di variabili minori
